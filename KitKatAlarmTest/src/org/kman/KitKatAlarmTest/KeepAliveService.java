...
import org.kman.KitKatAlarmTest.R;
import org.kman.tests.utils.HcNotificationBuilder;
...
public class KeepAliveService extends Service {
	...
	private static final int NOTIFICATION_ID = 0x00000002;
	private static final String EXTRA_PENDING_INTENT = "pendingIntent";

	public static class Info {
		private static final String EXTRA_MESSAGE = "message";

		public Info(CharSequence message) {
			mMessage = message;
		}

		public Info(Context context, int resId, String arg) {
			mMessage = context.getString(resId, arg);
		}
		...
		static Info unpack(Intent intent) {
			...
			if (!TextUtils.isEmpty(message)) {
				final Info info = new Info(message);
				return info;
			}
			return null;
		}
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		if (intent != null) {
			Info info = Info.unpack(intent);
			PendingIntent pendingIntent = intent.getParcelableExtra(EXTRA_PENDING_INTENT);

			if (info != null && pendingIntent != null) {
				/*
				 * Show a notification
				 */
				// MyLog.i(TAG, "onStartCommand msg = %s", info.mMessage);

				final long when = System.currentTimeMillis();
				Notification notification = null;

				final HcNotificationBuilder builder = HcNotificationBuilder.create(this, R.drawable.ic_launcher, null,
						when);
				if (builder != null) {
				...
				if (notification == null) {
					notification = new Notification(R.drawable.ic_launcher, null, when);
					notification.flags |= Notification.FLAG_ONGOING_EVENT;
					notification.setLatestEventInfo(this, getString(R.string.app_name), info.mMessage, pendingIntent);
				}

				startForeground(NOTIFICATION_ID, notification);
			} else {
				/*
				 * No message, remove the notification
				 */
			...
			/*
			 * No intent, we got restarted after getting kicked out of memory
			 */
